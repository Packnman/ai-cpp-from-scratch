...
float scheduled_learning_rate(const TrainingConfig &config,
    ...
    if (config.decay_updates > config.warmup_updates &&
        ...
        const double progress = static_cast<double>(update - config.warmup_updates) /
            static_cast<double>(config.decay_updates - config.warmup_updates);
        const double weight = 0.5 * (1.0 + std::cos(3.14159265358979323846 * progress));
        return static_cast<float>(config.minimum_learning_rate +
    ...
    if (is_sft) {
        ...
        manifest["sft_mix_weights"] =
            training.sft_profile() == "agent"
                ? nlohmann::json{{"CHAT", 20},      {"PARSE", 20},
                                 {"PLAN", 20},      {"EVALUATE", 10},
                                 {"SUMMARIZE", 10}, {"MEMORY_WRITE", 10},
                                 {"FINAL", 10}}
                : nlohmann::json{{"CHAT", 100}};
        manifest["trained_modes"] = training.trained_modes();
        manifest["sft_profile"] = training.sft_profile();
        manifest["sft_generation"] = {{"generator", "local-template-v2"},
...
void pretrain_sharded(AgentTransformer &model, const AgentTokenizer &tokenizer,
    ...
    const auto initial_device_used = initial_memory.deviceTotalBytes -
                                     initial_memory.deviceFreeBytes;
    const auto non_pool_device_bytes =
        initial_device_used > initial_memory.usedBytes
            ? initial_device_used - initial_memory.usedBytes : 0;
    std::mt19937 shuffle(
    ...
    for (; next_shard < stop; ++next_shard) {
        ...
        if (config.training.max_device_bytes &&
            ...
            throw std::runtime_error("estimated peak VRAM exceeds configured limit");
