...
AgentAttention::AgentAttention(int embedding, int heads, float dropout,
                               std::uint64_t seed)
    : _embedding(embedding), _heads(heads),
      _head_size(heads ? embedding / heads : 0), _dropout(dropout),
      _dropout_counter(seed), _query_weight(embedding, embedding),
      _query_bias(embedding, 1), _key_weight(embedding, embedding),
      _key_bias(embedding, 1), _value_weight(embedding, embedding),
      _value_bias(embedding, 1), _output_weight(embedding, embedding),
      _output_bias(embedding, 1), _query(&_query_weight, &_query_bias),
      _key(&_key_weight, &_key_bias), _value(&_value_weight, &_value_bias),
...
AgentFeedForward::AgentFeedForward(int embedding, int hidden, float dropout,
                                   std::uint64_t seed)
    : _embedding(embedding), _hidden(hidden), _dropout(dropout),
      _dropout_counter(seed), _input_weight(hidden, embedding),
      _input_bias(hidden, 1), _output_weight(embedding, hidden),
      _output_bias(embedding, 1), _input(&_input_weight, &_input_bias),
      _output(&_output_weight, &_output_bias) {
...
AgentTransformerBlock::AgentTransformerBlock(int embedding, int heads,
                                             ...
                                             std::uint64_t seed)
    : _gamma1(embedding, 1), _beta1(embedding, 1), _gamma2(embedding, 1),
      _beta2(embedding, 1), _attention_norm(&_gamma1, &_beta1),
...
TensorPtr AgentTransformerBlock::forward(TensorList &inputs) {
    ...
    auto first =
        _residual({inputs.at(0), _attention.forward(attention_inputs)});
...
AgentTransformer::AgentTransformer(const AgentTransformerConfig &config)
    : _config(config), _tokens(config.embedding, config.vocabulary),
      _positions(config.embedding, config.context), _gamma(config.embedding, 1),
      _beta(config.embedding, 1),
      _output_weight(config.vocabulary, config.embedding),
      _output_bias(config.vocabulary, 1), _token_embedding(&_tokens),
      _position_embedding(&_positions), _final_norm(&_gamma, &_beta),
      _output(&_output_weight, &_output_bias) {
    _config.validate();
...
TensorPtr AgentTransformer::forward(const std::shared_ptr<const cunMat> &ids) {
    ...
    for (std::size_t i = 0; i < position_ids.size(); ++i)
        position_ids[i] = static_cast<std::int32_t>(
            i / static_cast<std::size_t>(ids->size(1)));
    positions->copyFromHost(position_ids.data(), position_ids.size());
    auto result = _add_position(
        {_token_embedding(stable_ids), _position_embedding(positions)});
...
TensorPtr AgentTransformer::loss(const std::shared_ptr<const cunMat> &ids,
                                 const cunMat &targets, int ignored_id) {
    return graph_apply<IndexCrossEntropy>(TensorList{forward(ids)}, targets,
                                          ignored_id);
}
