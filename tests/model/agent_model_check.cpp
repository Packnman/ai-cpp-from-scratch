...
int main() {
    try {
        ...
        require(
            metadata.at("seed") == 19 &&
                metadata.at("sources").at("jawiki").at("scanned_documents") ==
                    1 &&
                metadata.at("sources").at("jawiki").at("selected_bytes") <= 32,
            "balanced tokenizer provenance");
        const auto efficiency =
            evaluate_tokenizer(balanced_loaded, root / "train.jsonl", 1);
        ...
        generate_sft_corpus(root / "conversation-train.jsonl",
                            root / "sft.jsonl", 19);
        const auto sft = AgentDataset::sft(root / "sft.jsonl", tokenizer, 1024);
        ...
        for (const auto index : sft.order(7, 3))
            ++sampled_modes[static_cast<std::size_t>(
                            ...
                            4];
        require(sampled_modes == std::array<std::size_t, 9>{20, 15, 15, 10, 20,
                                                            10, 0, 10, 0},
        ...
        resume_model_config.context = 1024;
...

        {
            ...
            const auto &last_validation =
                progress.at("validation_history").back();
            require(
                last_validation.at("bits_per_byte").get<double>() > 0.0 &&
