...
std::shared_ptr<Agent> make_agent(const std::filesystem::path &root,
                                  ...
                                  std::shared_ptr<ai::ner::IEntityExtractor> extractor = {}) {
    if (!reasoner)
    ...
    return std::make_shared<Agent>(
        ...
        std::make_shared<DefaultAggregator>(), memory, reasoner, extractor);
}
void parser_router() {
    RuleReasoner r;
...
void context_checks() {
    ...
    CHECK(x == b.build(in));
    in.require_summary = true;
    throws([&] { b.build(in); });
    CHECK(mode_prompt_tokens(ModelMode::Chat) == 766);
    CHECK(mode_prompt_tokens(ModelMode::Evaluate) == 894);
...
void model_checks() {
    ...
    const std::string valid =
        "[FACTS]\n- f\n[DECISIONS]\n- d\n[CONSTRAINTS]\n- c\n"
        "[OPEN_QUESTIONS]\n- q\n[ACTIVE_TASKS]\n- t\n[CORRECTIONS]\n- x";
    CHECK(validate_structured_summary(valid, 20));
    CHECK(!validate_structured_summary("[FACTS]\n- f", 2));
    auto summary_model = std::make_shared<ScriptModel>();
    summary_model->answers = {"invalid", valid};
    ModelReasoner summarizer(summary_model);
    CHECK(summarizer.summarize({{"u", "a"}}, "") == valid);
    CHECK(summary_model->modes ==
          std::vector<ModelMode>({ModelMode::Summarize, ModelMode::Summarize}));
}
void hybrid_e2e(const std::filesystem::path &dir) {
    ...
    auto failing = std::make_shared<ScriptModel>();
    const auto long_text = std::string(110, 'x');
    failing->answers = {long_text, long_text, long_text,
                        "invalid", "invalid", "履歴保持後の応答"};
    auto safe = make_agent(dir, dir / "hybrid-summary-failure.sqlite",
                           std::make_shared<HybridReasoner>(failing));
    CHECK(safe->process(long_text).success);
    CHECK(safe->process(long_text).success);
    CHECK(safe->process(long_text).success);
    const auto retained = safe->process("次の入力");
    CHECK(retained.success && retained.text == "履歴保持後の応答");
    CHECK(retained.state.recent_conversation.size() == 3);
    CHECK(retained.state.conversation_summary.empty());
    CHECK(failing->modes ==
          std::vector<ModelMode>({ModelMode::Chat, ModelMode::Chat,
                                  ModelMode::Chat, ModelMode::Summarize,
                                  ModelMode::Summarize, ModelMode::Chat}));
}
void ner_agent_e2e(const std::filesystem::path &dir) {
    auto agent = make_agent(dir, dir / "ner-agent.sqlite", {},
                            std::make_shared<ai::ner::RuleEntityExtractor>());
    auto response = agent->process("予算を5万円から3万円に変更。ただし承認までは確定しない。");
    CHECK(response.success);
    CHECK(response.state.entity_candidates.size() == 3);
    CHECK(response.state.entity_candidates[0].surface == "5万円");
    CHECK(response.state.entity_candidates[1].surface == "3万円");
    CHECK(response.state.entity_candidates[2].type == ai::ner::EntityType::Condition);
    CHECK(response.state.entity_candidates[0].utterance_id == "user:1");
}
