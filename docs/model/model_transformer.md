...
`conversation_check` の短系列・小規模過学習（80更新で loss < 0.1）、保存再読込の FP32 logits 完全一致、学習／評価／最良重みの test 評価が通ること。既定4ブロック・256・4ヘッド・1024・context 1024の実データ検証は [実測記録](conversation_validation.md) に記録する。
