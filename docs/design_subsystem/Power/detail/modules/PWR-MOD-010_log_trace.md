# PWR-MOD-010 Log / Trace 詳細設計
# 1. 目的
Power State、Protection Event、Shutdown Event、Branch操作を記録し解析・再現性を確保する。
# 2. 責務
- Periodic power telemetry log
- Warning / Fault event log
- Branch enable/disable log
- Current limit change log
- Battery / SOC log
- Shutdown sequence log
- Timestamp / correlation ID管理
- Flush要求処理
# 3. 非責務
...
# 6. 内部状態
```text
Fault / Shutdown Eventは通常Telemetryより高優先度で保持する。
Log失敗はPower Safetyそのものを停止させない。
```
# 7. 基本処理
```text
update()
...
を必要に応じ記録する。
# 12. 依存関係
State Interface、Shutdown Manager、Persistent Storage / Logger
# 13. Testとの分離
Test Caseは本書に定義しない。
