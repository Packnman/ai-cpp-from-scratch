# PWR-MOD-005 Current Monitor 詳細設計
# 1. 目的
Mainおよび各Actuator BranchのCurrentを監視しPower / Energy算出とOvercurrent判定材料を提供する。
# 2. 責務
- Main current sampling
...
PowerStateDerived
EnergyCounter
CurrentWarning / Fault
```
# 6. 内部状態
```text
Fast peakはHardware側、SoftwareはControl MonitoringとTelemetryを担当する。
```
# 7. 基本処理
```text
update()
...
# 8. Interface Concept
```cpp
class ICurrentMonitor
{
public:
    virtual ~ICurrentMonitor() =default;
...
を必要に応じ記録する。
# 12. 依存関係
Voltage Monitor、Protection Manager、Distribution Manager、State Interface
# 13. Testとの分離
Test Caseは本書に定義しない。
