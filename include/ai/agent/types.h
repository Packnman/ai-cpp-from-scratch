...
struct ParsedInput {
        ...
        nlohmann::json arguments{
            nlohmann::json::object()}; // 抽出した構造化引数
        std::vector<ai::ner::EntityMention>
            entity_candidates; // 根拠付きNER候補（事実確定前）
};
...
struct ConversationTurn {
        ...
        std::vector<ai::ner::EntityMention>
            entity_candidates; // 当該発話の抽出候補
};

struct AgentState {
        RequestType request_type{
            RequestType::SimpleConversation}; // 現在の要求種別
        ...
        std::vector<ai::ner::EntityMention>
            entity_candidates;                    // 現入力の抽出候補
        nlohmann::json discussion_state{nullptr}; // 限定比較の構造化状態
};
