...
class AgentDataset {
public:
    static AgentDataset jawiki(
        const std::filesystem::path &train_jsonl,
        const AgentTokenizer &tokenizer,
        std::size_t context,
        std::size_t token_limit = 0
    );
    static AgentDataset sft(
        const std::filesystem::path &train_jsonl,
        const AgentTokenizer &tokenizer,
        std::size_t context
    );

    const std::vector<AgentWindow> &windows() const noexcept { return _windows; }
    ...
    const std::vector<std::string> &trained_modes() const noexcept { return _trained_modes; }
    std::uint64_t generation_seed() const noexcept { return _generation_seed; }
...
private:
    ...
    std::string _sft_profile, _split; // SFT生成プロファイルとデータセット分割名
    std::vector<std::string> _trained_modes; // コーパスに含まれる学習モード
    std::uint64_t _generation_seed = 0; // コーパス生成時の乱数シード（未記録時は0）
    std::size_t _input_bytes = 0; // loss対象の元UTF-8入力byte数
};

void generate_sft_corpus(
